...
export default function ViewCreator({ onClose }: ViewCreatorProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log('Catégories spéciales:', specialCategories);
        console.log('Items récupérés:', itemsList);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fields = selectedFields.map(fieldId => {
      const item = items.find(i => i.id === fieldId);
      const category = categories.find(c => c.id === fieldId);
      const fieldData = item || category;
      
      return {
        id: fieldId,
        name: fieldData?.text || fieldData?.name || '',
    ...
    await addView({
      ...
      fields,
      settings: {
        showLastModified: selectedFields.includes(categories.find(cat => cat.name === 'lastModified')?.id || '')
      }
    });
